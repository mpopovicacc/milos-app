...
const [isInEState, setIsInEState] = useState(false); // Track if in E state
const [ctrlPressed, setCtrlPressed] = useState(false); // Flag for detecting Ctrl press	
...
useEffect(() => {
  ...
  if (isCtrlPressed) {
    ...
    setCtrlPressDuration(0); // Reset immediately when not pressing
  }
...
useEffect(() => {
  ...
  const countdown = setInterval(() => {
    setTimeLeft(prev => {
      if (isInEState) return prev; // stop even mid-loop
      ...
      if (prev <= 1) {
        ...
        setCurrentState('E');
        setIndex(0);
        setIsInEState(true);      // Lock into E
...
const handleCtrlClick = () => {
  if (isInEState) return; // ← BLOCK input if in E
