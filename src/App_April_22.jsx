...
import React, { useRef } from 'react'; // Remove the second `useEffect` import
import { useEffect, useState } from 'react'; // Keep `useEffect` and `useState` here
...
function App() {
  ...
  const [pressStatus, setPressStatus] = useState("CTRL Btn");
...
/*     // Set the Ctrl button as not pressed
    ...
    if (pressDuration < 1000) {
      ...
      setPressStatus("CTRL Btn"); // Reset state after release	  
    }
  };
...
>
        Ctrl
	        ...
	        {/* Absolute positioned press status */}
			{pressStatus && (
			  <div style={{
				position: "fixed",
				top: "50%", // Center vertically
				left: "50%", // Center horizontally
				transform: "translate(-70%, 100%)", // Adjust for perfect centering
				background: "yellow",
				padding: "5px",
				borderRadius: "5px",
				fontSize: "10px",
				boxShadow: "2px 2px 10px rgba(0,0,0,0.2)" // Optional styling for better visibility
			  }}>
			Click CTRL button to cycle through pages<br /><br />
			Long Click CTRL button for > 2S CTRL button to exit Error state<br /><br />
			
          {pressStatus} <br />
		  isInEState: {isInEState.toString()}<br />
		  currentState: {currentState.toString()}<br />
